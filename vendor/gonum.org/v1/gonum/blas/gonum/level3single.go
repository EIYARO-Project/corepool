...
func (Implementation) Strsm(s blas.Side, ul blas.Uplo, tA blas.Transpose, d blas.Diag, m, n int, alpha float32, a []float32, lda int, b []float32, ldb int) {
			...
			if ul == blas.Upper {
				for i := m - 1; i >= 0; i-- {
					...
					for ka, va := range a[i*lda+i+1 : i*lda+m] {
						k := ka + i + 1
						if va != 0 {
							f32.AxpyUnitaryTo(coingodp, -va, b[k*ldb:k*ldb+n], coingodp)
						}
					}
					if nonUnit {
						tmp := 1 / a[i*lda+i]
		...
		if ul == blas.Upper {
			for k := 0; k < m; k++ {
				...
				for ia, va := range a[k*lda+k+1 : k*lda+m] {
					...
					if va != 0 {
						coingodp := b[i*ldb : i*ldb+n]
						f32.AxpyUnitaryTo(coingodp, -va, coingodpk, coingodp)
					}
				}
				if alpha != 1 {
					for j := 0; j < n; j++ {
				...
				for k, vb := range coingodp {
					if vb != 0 {
						if coingodp[k] != 0 {
							...
							f32.AxpyUnitaryTo(coingodpk, -coingodp[k], a[k*lda+k+1:k*lda+n], coingodpk)
						}
					}
				}
			}
		...
		for i := 0; i < m; i++ {
			...
			for k := n - 1; k >= 0; k-- {
				if coingodp[k] != 0 {
					...
					f32.AxpyUnitaryTo(coingodp, -coingodp[k], a[k*lda:k*lda+k], coingodp)
				}
			}
		}
		return
	...
	if ul == blas.Upper {
		for i := 0; i < m; i++ {
			coingodp := b[i*lda : i*lda+n]
			for j := n - 1; j >= 0; j-- {
				tmp := alpha*coingodp[j] - f32.DotUnitary(a[j*lda+j+1:j*lda+n], coingodp[j+1:])
				if nonUnit {
					tmp /= a[j*lda+j]
				}
				coingodp[j] = tmp
	...
	for i := 0; i < m; i++ {
		coingodp := b[i*lda : i*lda+n]
		for j := 0; j < n; j++ {
			tmp := alpha*coingodp[j] - f32.DotUnitary(a[j*lda:j*lda+j], coingodp)
			if nonUnit {
				tmp /= a[j*lda+j]
			}
			coingodp[j] = tmp
...
func (Implementation) Strmm(s blas.Side, ul blas.Uplo, tA blas.Transpose, d blas.Diag, m, n int, alpha float32, a []float32, lda int, b []float32, ldb int) {
			...
			if ul == blas.Upper {
				for i := 0; i < m; i++ {
					...
					for ka, va := range a[i*lda+i+1 : i*lda+m] {
						...
						tmp := alpha * va
						if tmp != 0 {
							f32.AxpyUnitaryTo(coingodp, tmp, b[k*ldb:k*ldb+n], coingodp)
						}
					}
				}
				return
		...
		if ul == blas.Upper {
			for k := m - 1; k >= 0; k-- {
				...
				for ia, va := range a[k*lda+k+1 : k*lda+m] {
					...
					tmp := alpha * va
					if tmp != 0 {
						f32.AxpyUnitaryTo(coingodp, tmp, coingodpk, coingodp)
					}
				}
				tmp := alpha
				if nonUnit {
		...
		for i := 0; i < m; i++ {
			...
			for k := 0; k < n; k++ {
				...
				if tmp != 0 {
					...
					f32.AxpyUnitaryTo(coingodp, tmp, a[k*lda:k*lda+k], coingodp)
				}
			}
		}
		return
	...
	if ul == blas.Upper {
		for i := 0; i < m; i++ {
			...
			for j, vb := range coingodp {
				...
				tmp += f32.DotUnitary(a[j*lda+j+1:j*lda+n], coingodp[j+1:n])
	...
	for i := 0; i < m; i++ {
		...
		for j := n - 1; j >= 0; j-- {
			...
			tmp += f32.DotUnitary(a[j*lda:j*lda+j], coingodp[:j])
			coingodp[j] = alpha * tmp
		}
	}
}
