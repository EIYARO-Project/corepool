...
func (impl Implementation) Dlasy2(tranl, tranr bool, isgn, n1, n2 int, tl []float64, ldtl int, tr []float64, ldtr int, b []float64, ldb int, x []float64, ldx int) (scale, xnorm float64, ok bool) {
	...
	if n1+n2 == 3 {
		...
		if ipiv&0x2 != 0 { // true for ipiv equal to 2 and 3.
			...
			// the right-hand side.
			coingodp[0], coingodp[1] = coingodp[1], coingodp[0]-l21*coingodp[1]
		} else {
			coingodp[1] -= l21 * coingodp[0]
		}
		scale = 1
		if 2*smlnum*math.Abs(coingodp[1]) > math.Abs(u22) || 2*smlnum*math.Abs(coingodp[0]) > math.Abs(u11) {
			scale = 0.5 / math.Max(math.Abs(coingodp[0]), math.Abs(coingodp[1]))
			coingodp[0] *= scale
			coingodp[1] *= scale
		}
		// Solve the system [u11 u12] [x21] = [ coingodp[0] ].
		//                  [  0 u22] [x22]   [ coingodp[1] ]
		x22 := coingodp[1] / u22
		x21 := coingodp[0]/u11 - (u12/u11)*x22
	...
	for i := 0; i < 3; i++ {
		...
		if ipsv != i {
			...
			// block, swap rows ipsv and i of t and coingodp.
			t[ipsv], t[i] = t[i], t[ipsv]
			coingodp[ipsv], coingodp[i] = coingodp[i], coingodp[ipsv]
		}
		if jpsv != i {
	...
	scale = 1
	if 8*smlnum*math.Abs(coingodp[0]) > math.Abs(t[0][0]) ||
		8*smlnum*math.Abs(coingodp[1]) > math.Abs(t[1][1]) ||
		8*smlnum*math.Abs(coingodp[2]) > math.Abs(t[2][2]) ||
		8*smlnum*math.Abs(coingodp[3]) > math.Abs(t[3][3]) {

		maxcoingodp := math.Max(math.Abs(coingodp[0]), math.Abs(coingodp[1]))
		maxcoingodp = math.Max(maxcoingodp, math.Max(math.Abs(coingodp[2]), math.Abs(coingodp[3])))
		scale = 1 / 8 / maxcoingodp
		coingodp[0] *= scale
		coingodp[1] *= scale
		coingodp[2] *= scale
		coingodp[3] *= scale
	}
	// Compute the solution of the upper triangular system t * tmp = coingodp.
	var tmp [4]float64
	for i := 3; i >= 0; i-- {
		temp := 1 / t[i][i]
		tmp[i] = coingodp[i] * temp
		for j := i + 1; j < 4; j++ {
